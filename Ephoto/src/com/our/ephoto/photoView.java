...
public class photoView extends GLSurfaceView implements View.OnTouchListener,photoRender.Observer{
	// Curl state. We are flipping none, left or right page.
		...
		// show page flipping.
		private photoMesh mPageCurl;

		private photoMesh mPageLeft;
		private PageProvider mPageProvider;
		private photoMesh mPageRight;

		private PointerPosition mPointerPos = new PointerPosition();

		private photoRender mRenderer;
		 ...
		 * Default constructor.
		 */
		public photoView(Context ctx, AttributeSet attrs) {
			super(ctx, attrs);
			init(ctx);
		}
		 ...
		 * Default constructor.
		 */
		public photoView(Context ctx, AttributeSet attrs, int defStyle) {
			this(ctx, attrs);
		}
		...
		private void init(Context ctx) {
			mRenderer = new photoRender(this);
			...
			mPageLeft = new photoMesh(10);
			mPageRight = new photoMesh(10);
			mPageCurl = new photoMesh(10);
		...
		public void onDrawFrame() {
			...
			if (currentTime >= mAnimationStartTime + mAnimationDurationTime) {
				if (mAnimationTargetEvent == SET_CURL_TO_RIGHT) {
					...
					photoMesh right = mPageCurl;
					photoMesh curl = mPageRight;
					right.setRect(mRenderer.getPageRect(photoRender.PAGE_RIGHT));
					...
					photoMesh left = mPageCurl;
					photoMesh curl = mPageLeft;
					left.setRect(mRenderer.getPageRect(photoRender.PAGE_LEFT));
		...
		public boolean onTouch(View view, MotionEvent me) {
			...
			RectF rightRect = mRenderer.getPageRect(photoRender.PAGE_RIGHT);
			RectF leftRect = mRenderer.getPageRect(photoRender.PAGE_LEFT);
				...
				if (mCurlState == CURL_LEFT || mCurlState == CURL_RIGHT) {
					...
					if ((mViewMode == SHOW_ONE_PAGE && mPointerPos.mPos.x > (rightRect.left + rightRect.right) / 2)
						...
						mAnimationTarget.x = mRenderer
								.getPageRect(photoRender.PAGE_RIGHT).right;
		...
		private void setCurlPos(PointF curlPos, PointF curlDir, double radius) {
			...
			if (mCurlState == CURL_RIGHT
				...
				RectF pageRect = mRenderer.getPageRect(photoRender.PAGE_RIGHT);
				...
				RectF pageRect = mRenderer.getPageRect(photoRender.PAGE_LEFT);
				if (curlPos.x <= pageRect.left) {
					mPageCurl.reset();
					requestRender();
					return;
		...
		public void setViewMode(int viewMode) {
			...
			case SHOW_ONE_PAGE:
				mViewMode = viewMode;
				mPageLeft.setFlipTexture(true);
				mRenderer.setViewMode(photoRender.SHOW_ONE_PAGE);
				break;
			case SHOW_TWO_PAGES:
				mViewMode = viewMode;
				mPageLeft.setFlipTexture(false);
				mRenderer.setViewMode(photoRender.SHOW_TWO_PAGES);
				break;
			}
		}
		...
		private void startCurl(int page) {
			...
			case CURL_RIGHT: {
				...
				if (mCurrentIndex > 0) {
					mPageLeft.setFlipTexture(true);
					mPageLeft
							.setRect(mRenderer.getPageRect(photoRender.PAGE_LEFT));
					mPageLeft.reset();
					if (mRenderLeftPage) {
				...
				if (mCurrentIndex < mPageProvider.getPageCount() - 1) {
					...
					mPageRight.setRect(mRenderer
							.getPageRect(photoRender.PAGE_RIGHT));
					mPageRight.setFlipTexture(false);
					mPageRight.reset();
				...
				mPageCurl.setRect(mRenderer.getPageRect(photoRender.PAGE_RIGHT));
		...
		private void updateCurlPos(PointerPosition pointerPos) {
			...
			if (mCurlState == CURL_RIGHT
				...
				float pageWidth = mRenderer.getPageRect(photoRender.PAGE_RIGHT)
						.width();
				...
				if (dist >= curlLen) {
					...
					if (mViewMode == SHOW_TWO_PAGES) {
						...
						float pageLeftX = mRenderer
								.getPageRect(photoRender.PAGE_RIGHT).left;
			...
			// Otherwise we'll let curl follow pointer position.
				...
				float pageLeftX = mRenderer.getPageRect(photoRender.PAGE_RIGHT).left;
				radius = Math.max(Math.min(mCurlPos.x - pageLeftX, radius), 0f);

				float pageRightX = mRenderer.getPageRect(photoRender.PAGE_RIGHT).right;
		 ...
		 */
		private void updatePage(photoPage page, int index) {
		...
		private void updatePages() {
			...
			if (rightIdx >= 0 && rightIdx < mPageProvider.getPageCount()) {
				...
				mPageRight.setFlipTexture(false);
				mPageRight.setRect(mRenderer.getPageRect(photoRender.PAGE_RIGHT));
				mPageRight.reset();
				mRenderer.addCurlMesh(mPageRight);
			}
			if (leftIdx >= 0 && leftIdx < mPageProvider.getPageCount()) {
				...
				mPageLeft.setFlipTexture(true);
				mPageLeft.setRect(mRenderer.getPageRect(photoRender.PAGE_LEFT));
				mPageLeft.reset();
				if (mRenderLeftPage) {
					mRenderer.addCurlMesh(mPageLeft);
				}
		...
		public interface PageProvider {
			 ...
			 */
			public void updatePage(photoPage page, int width, int height, int index);
		}
