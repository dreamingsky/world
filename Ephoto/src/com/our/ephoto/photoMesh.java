...
public class photoMesh {
	// Flag for rendering some lines used for developing. Shows
		...
		private final photoPage mTexturePage = new photoPage();
		 ...
		 */
		public photoMesh(int maxCurlSplits) {
			...
			for (int i = 0; i < mArrScanLines.size(); ++i) {
				...
				while (mArrOutputVertices.size() > 0) {
					...
					if (textureFront != mFlipTexture) {
						...
						v.mColor = mTexturePage.getColor(photoPage.SIDE_FRONT);
					} else {
						v.mTexX *= mTextureRectBack.right;
						v.mTexY *= mTextureRectBack.bottom;
						v.mColor = mTexturePage.getColor(photoPage.SIDE_BACK);
					}
		 ...
		 */
		public synchronized photoPage getTexturePage() {
			return mTexturePage;
		}
		...
		public synchronized void reset() {
			...
			for (int i = 0; i < 4; ++i) {
				...
				if (mFlipTexture) {
					...
					tmp.mColor = mTexturePage.getColor(photoPage.SIDE_BACK);
				} else {
					tmp.mTexX *= mTextureRectFront.right;
					tmp.mTexY *= mTextureRectFront.bottom;
					tmp.mColor = mTexturePage.getColor(photoPage.SIDE_FRONT);
				}
